...
export default {
  apply(ctx) {
    ...
    if (root === null) {
      ...
      if (home) root = home + '/.dsh/background'
    }
    const CFG_FILE = root ? root + '/.dsh-bg-config.json' : '.dsh-bg-config.json'
    const imgFile = (id) => (root ? root + '/.dsh-bg-' + id + '.json' : '.dsh-bg-' + id + '.json')
    ...
    const writeImage = async (id, mime, bytes) => {
      if (fs === undefined) return false
      const target = await fs.resolve(imgFile(id))
      await fs.writeText(target, JSON.stringify({ mime, data: bytesToBase64(bytes) }))
      return true
    }
    ...
    const readConfig = async () => {
      if (fs === undefined) return null
      const target = await fs.resolve(CFG_FILE)
      const info = await fs.stat(target)
      if (info === undefined) return null
      const text = await fs.readText(target)
    ...
    const writeConfig = async (cfg) => {
      if (fs === undefined) return false
      const target = await fs.resolve(CFG_FILE)
      await fs.writeText(target, JSON.stringify(cfg === undefined ? null : cfg, null, 2))
      return true
    }
      ...
      disposers.push(ws.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            if (op === 'load') return sendJson(res, 200, { ok: true, data: await readConfig() })
            if (op === 'save') {
              await writeConfig(args === undefined ? null : args)
              return sendJson(res, 200, { ok: true })
            }
            if (op === 'delete') {
              const id = args && typeof args.id === 'string' ? args.id : ''
              if (!id) return sendJson(res, 400, { ok: false, error: 'id' })
              await fs.resolve(imgFile(id)).then((t) => fs.writeText(t, ''))
              return sendJson(res, 200, { ok: true })
            }
      ...
      disposers.push(ws.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            const id = uid()
            const stored = await writeImage(id, mime, fileBytes)
    ...
    // webServer may not exist yet when rows apply early at boot: register
    // lazily once it appears (retry with the cordis timer mixin).
